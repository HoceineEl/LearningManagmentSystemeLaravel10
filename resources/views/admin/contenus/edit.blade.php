...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.contenu.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contenus.update", [$contenu->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lesson') ? 'is-invalid' : '' }}" name="lesson_id" id="lesson_id" required>
                    @foreach($lessons as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lesson_id') ? old('lesson_id') : $contenu->lesson->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ordre') ? 'is-invalid' : '' }}" type="number" name="ordre" id="ordre" value="{{ old('ordre', $contenu->ordre) }}" step="1" required>
                @if($errors->has('ordre'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ordre') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\Contenu::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $contenu->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('id_type') ? 'is-invalid' : '' }}" type="number" name="id_type" id="id_type" value="{{ old('id_type', $contenu->id_type) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $contenu->description) !!}</textarea>
