...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.utilisateurReponse.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.utilisateur-reponses.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reponse') ? 'is-invalid' : '' }}" name="reponse_id" id="reponse_id" required>
                    @foreach($reponses as $id => $entry)
                        <option value="{{ $id }}" {{ old('reponse_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
