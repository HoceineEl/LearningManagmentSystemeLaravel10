...
@section('content')
    @can('question_reponse_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                @isset($question)
                    <a class="btn btn-success" href="{{ url('admin/question-reponses/create/' . $question->id) }}">
                            ...
                            <tr data-entry-id="{{ $questionReponse->id }}">
                                ...
                                <td>
                                    ...
                                    <input type="checkbox" disabled="disabled"
                                        {{ $questionReponse->est_correct ? 'checked' : '' }}>
                                </td>
                                <td>
                                    @can('question_reponse_show')
                                        <a class="btn btn-xs btn-primary"
                                            href="{{ route('admin.question-reponses.show', $questionReponse->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    ...
                                    @can('question_reponse_edit')
                                        <a class="btn btn-xs btn-info"
                                            href="{{ route('admin.question-reponses.edit', $questionReponse->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('question_reponse_delete')
                                        <form action="{{ route('admin.question-reponses.destroy', $questionReponse->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-QuestionReponse:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
