        ...
        <div class="col-md-12">
            @can('quiz_question_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.quiz-questions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $quizQuestion->id }}">
                                        ...
                                        <td>
                                            @can('quiz_question_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.quiz-questions.show', $quizQuestion->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('quiz_question_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.quiz-questions.edit', $quizQuestion->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('quiz_question_delete')
                                                <form action="{{ route('frontend.quiz-questions.destroy', $quizQuestion->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('quiz_question_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.quiz-questions.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-QuizQuestion:not(.ajaxTable)').DataTable({ buttons: dtButtons })
