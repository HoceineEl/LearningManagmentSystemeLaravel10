        ...
        <div class="col-md-12">
            @can('lecon_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.lecons.create') }}">
                            ...
                            <tbody>
                                @foreach($lecons as $key => $lecon)
                                    <tr data-entry-id="{{ $lecon->id }}">
                                        ...
                                        <td>
                                            @can('lecon_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.lecons.show', $lecon->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('lecon_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.lecons.edit', $lecon->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan
...
@can('lecon_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.lecons.massDestroy') }}",
