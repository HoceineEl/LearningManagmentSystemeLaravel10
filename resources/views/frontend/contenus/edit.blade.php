    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.contenus.update", [$contenu->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="lecon_id" id="lecon_id" required>
                                @foreach($lecons as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('lecon_id') ? old('lecon_id') : $contenu->lecon->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('lecon'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="ordre" id="ordre" value="{{ old('ordre', $contenu->ordre) }}" step="1" required>
                            @if($errors->has('ordre'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('ordre') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="id_type" id="id_type" value="{{ old('id_type', $contenu->id_type) }}" step="1">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $contenu->description) !!}</textarea>
